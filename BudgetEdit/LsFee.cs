...
namespace BudgetEdit
{
    ...
    public class LsFeeItem
    {
        ...
        public LsFeeItem(string sProject, string sNumber, string sFeeName, string sUnit, string sCount, string sUnit_price, string sPrice, string sRemark, string sTotal)
        {
    ...
    public class LsFee
    {
        ...
        /*********************************更新dataTable递增标识列为具有父子关系的标识列******************************************************/
        public LsFee(string proName, string bidNumber)
        {
            string sNumber, sName, sUnit, sCount, sUnitPrice, sPrice, sRemark;
            lsfs = new List<LsFeeItem>();
            List<string> list_number = new List<string>();
            string totalSum;
            string sql = "select 标识,父亲,费用项目,单位,数量,单价,合价,备注 from linshi_fee where 所属标段='" + bidNumber + "' order by 标识 asc";                      //正式确认时sql语句需添加where+bid_name分支以确定对应标段
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                lsfs.Add(new LsFeeItem(proName, sNumber, sName, sUnit, sCount, sUnitPrice, sPrice, sRemark, totalSum));
            }
        }

        public List<LsFeeItem> GetItems()
        {
            return lsfs;
        }
    }
